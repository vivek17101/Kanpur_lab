...
function Icon({ type, className = 'h-5 w-5' }) {
  ...
  if (type === 'success') {
    return (
      <svg {...common}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15.75 9M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
      </svg>
    );
  }
  ...
  if (type === 'users') {
    return (
      <svg {...common}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.13a7.5 7.5 0 0 0-6 0M15 11a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 8.25a5.25 5.25 0 0 0-7.5-4.75M3 19.25a5.25 5.25 0 0 1 7.5-4.75" />
      </svg>
    );
  }
  ...
  return (
    <svg {...common}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2.25 5H6.75A2.25 2.25 0 0 1 4.5 18.75V5.25A2.25 2.25 0 0 1 6.75 3h6.88c.6 0 1.17.24 1.59.66l3.12 3.12c.42.42.66.99.66 1.59v10.38A2.25 2.25 0 0 1 16.75 21Z" />
    </svg>
  );
}
...
function StatCard({ label, value, tone = 'neutral', icon, helper }) {
  const toneClass = {
    neutral: 'bg-slate-50 text-slate-700 ring-slate-200 dark:bg-slate-800/70 dark:text-slate-200 dark:ring-slate-700',
  ...
  return (
    <article className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-soft transition duration-200 hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
      ...
      {helper && <p className="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">{helper}</p>}
    </article>
  );
}
...
export default function Dashboard({ onNavigate }) {
  ...
  const filteredSamples = useMemo(() => {
    ...
    return recentSamples.filter((sample) =>
      [sample.reportNumber, sample.sampleNo, sample.supplierName, sample.sampleReference, sample.status]
        .filter(Boolean)
  ...
  return (
    <section className="space-y-6">
        ...
        <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300">
              Track incoming samples, pending result entry, report-ready work, and supplier coverage from one focused dashboard.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total samples" value={stats.total} icon="document" helper="All registered samples" />
        <StatCard label="Need attention" value={attentionCount} tone="warning" icon="warning" helper="Pending or ready to report" />
        <StatCard label="Reports sent" value={stats.Reported} tone="success" icon="success" helper="Marked as reported" />
        <StatCard label="Suppliers" value={supplierCount} tone="info" icon="users" helper="Saved supplier records" />
      </div>
          ...
          <div className="mt-5 space-y-3">
            {[
              ...
              >
                ...
                <strong className="w-12 text-2xl font-bold text-slate-950 dark:text-white">{value || 0}</strong>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-4 border-b border-slate-200 p-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Click a row to continue in the register.</p>
            </div>
                ...
                {filteredSamples.length === 0 ? (
                  ...
                  filteredSamples.map((sample) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{sample.supplierName}</td>
                      <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{sample.sampleReference}</td>
                      <td className="px-5 py-4"><StatusBadge status={sample.status} /></td>
