...
function App() {
  ...
  useEffect(() => {
    async function restoreSession() {
      if (!getStoredToken()) { setIsCheckingSession(false); return; }
      try {
  ...
  const handleBackupDatabase = async () => {
    ...
    try {
      ...
      link.href = url; link.download = `kanpur-lab-backup-${timestamp}.json`; link.click();
      URL.revokeObjectURL(url);
      showToast("Database backup downloaded.", "success");
    } catch (err) {
      showToast(err.message || "Could not create database backup.", "error");
    } finally {
      setIsBackingUp(false);
    }
  };

  const handleRestoreClick = () => { if (!isRestoring) restoreInputRef.current?.click(); };
  ...
  const handleRestoreFileChange = async (event) => {
    ...
    setConfirm({
      ...
      detail: "This will replace all current samples, suppliers, and report counters. This cannot be undone.",
      onConfirm: async () => {
        dismissConfirm();
        setIsRestoring(true);
        try {
          const result = await restoreDatabase(pendingBackupRef.current);
          showToast(`Restored — ${result.counts.samples} samples, ${result.counts.suppliers} suppliers.`, "success");
  ...
  return (
    <div className="App">
      ...
      <ConfirmModal
        ...
        variant="primary"
      />
      <input ref={restoreInputRef} type="file" accept="application/json,.json" style={{ display: "none" }} onChange={handleRestoreFileChange} />
          ...
          <>
            <div className={styles.topbar}>
              <nav className={styles.nav}>
                <button className={activePage === "register" ? styles.active : ""} onClick={() => setActivePage("register")}>Sample Register</button>
                <button className={activePage === "suppliers" ? styles.active : ""} onClick={() => setActivePage("suppliers")}>Supplier Master</button>
                <button className={activePage === "password" ? styles.active : ""} onClick={() => setActivePage("password")}>Change Password</button>
              </nav>
              <div className={styles.adminBox}>
                <span>Signed in as {admin.username}</span>
                <Button size="md" variant="secondary" onClick={handleBackupDatabase} disabled={isBackingUp || isRestoring}>
                  <ButtonLabel label={isBackingUp ? "Backing Up..." : "Backup Database"} />
                </Button>
                <Button size="md" variant="secondary" onClick={handleRestoreClick} disabled={isRestoring || isBackingUp}>
            ...
            {activePage === "suppliers" && <SupplierManager onSuppliersChanged={() => setSuppliersVersion((v) => v + 1)} />}
